...
| `Project page →` | 이 사이트 안의 상세 페이지 | **Demos·News 페이지에서만** |
| `News ↗` | 연구실 소식 게시물 | **News 페이지에서만** |
...
5. 링크는 `News ↗`(연구실 소식) 하나까지. 논문 페이지로 연결하지 않는다 —
   Publications가 nav 한 번 거리에 있고, 11개 중 3개에만 붙던 예외였다.
