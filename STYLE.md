...
- **`?v=` 해시만 바뀐 줄도 같이 커밋한다.** 이 값은 자산 내용의 sha1이고 캐시 키 그
  자체라, 빼고 커밋하면 다음 사람에게 계속 따라붙는다. `--check`가 out-of-sync라고
  하면 `build_site.py`를 돌려 맞춘 뒤 그 줄까지 함께 커밋한다.
  **`audit_site.py`가 모든 토큰을 자산의 실제 해시와 대조**하므로 이제 조용히 어긋날 수 없다.

  > **왜 이게 계속 어긋나 보였나 (2026-08-14 해결).** `core.autocrlf=true`라 작업 사본은
  > CRLF, 저장소(=GitHub Pages가 서빙하는 것)는 LF다. `asset_version()`이 디스크의
  > 바이트를 그대로 해시하는 바람에 **같은 파일이 Windows 체크아웃에서는 `959353f1`,
  > 라이브에서는 `cf07c318`** 로 나왔다. 두 값이 빌드할 때마다 서로를 덮어써서 `--check`가
  > 영원히 시끄러웠고, 그래서 **그 줄을 빼고 커밋하는 습관**이 생겼다. 내용이 바뀐 적은
  > 없었다 — 줄끝만 다른 같은 파일이었다. `asset_version()`이 이제 `\r\n`→`\n`으로
  > 정규화한 뒤 해시하므로 토큰이 **기계가 아니라 자산의 성질**이 됐다.
  > **다른 곳에서 자산 해시를 계산할 일이 생기면 같은 정규화를 해야 한다.**
