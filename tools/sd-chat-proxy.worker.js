 ...
 *
 * The site is served from GitHub Pages, which is static. A key in the page's
 * JavaScript is a key every visitor can read and every scraper can take, so the
 * key lives here as an encrypted Worker secret and the browser only ever talks
 * to this endpoint.
 *
 * Deploy: Cloudflare dashboard -> Compute -> Workers & Pages -> Create -> Worker.
 * Paste this in, deploy once, then Settings -> Variables and Secrets -> add
 * OPENAI_API_KEY as a Secret (not a plaintext variable). Deploy again.
 *
 ...
 *   - only this site's origin is allowed,
...
const ALLOWED_ORIGIN = 'https://bogyeompark.github.io';
